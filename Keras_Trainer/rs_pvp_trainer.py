...
def parseDataSet(data_set_path):
    data_x = []
    data_y = []
    time_step = 10
    max_a = 0
...
json_file = open(model_name_dense, 'r')
loaded_model_json = json_file.read()
json_file.close()

model = model_from_json(loaded_model_json)

if(os.path.exists(save_weights_dense_file)):
    print("already exsits")
    model.load_weights(save_weights_dense_file) 
else:
    print("does not exist")

opt = SGD(lr=0.0001, decay=0.000001, momentum=0.9, nesterov=True)
...
if(traning_mode == True):
    print("Traning Mode")
    data_x, data_y_useless = parseDataSet(data_set_1)
   ...
   if(predict_mode == True):
       ...
       data_x_useless_1, data_y_1 = get_raw_data(12000, data_set_1, 6, 3)
       pre = model.predict(np.array(data_x_useless_1))
       correct = 0
       for i in range(0,len(pre)):
           if(np.argmax(pre[i]) == np.argmax(data_y_1[i])):
               if(np.argmax(pre[i]) == 0 and data_x_useless_1[i][1]==-1):
                   print(data_x_useless_1[i],"  ",pre[i])    
               correct = correct +1
       print(correct)
   else:
       print("Testing Mode") 
